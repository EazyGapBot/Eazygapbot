...
ADMIN_ID = 1624053514  # مالک اصلی ربات (سوپر ادمین)
DB_PATH = "bot.db"
...
# سکه پیش‌فرض هدیه‌ی ورود اولیه به ربات
DEFAULT_START_COINS = 10
# سکه هدیه تکمیل پروفایل (یکبار، فقط پس از تکمیل ثبت‌نام)
PROFILE_COMPLETE_BONUS = 10
# سکه هدیه معرفی هر دوست (وقتی شخص دعوت‌شده وارد ربات شد)
REFERRAL_BONUS = 15

# بازه‌ی تصادفی که هر بار به قیمت افزوده می‌شود (به تومان) — جلوگیری از رسید تکراری/فیک
RANDOM_PRICE_MIN = 1
...
def to_en_digits(text: str) -> str:
    """هر عدد فارسی/عربی داخل متن ورودی کاربر رو به عدد انگلیسی تبدیل می‌کنه."""
    if not text:
        return text
    table = {}
...
PROVINCES = {
    ...
    "آذربایجان شرقی": [
        "تبریز", "مراغه", "میانه", "مرند", "اهر", "بناب", "سراب", "شبستر", "هشترود",
        "کلیبر", "جلفا", "ورزقان", "هریس", "بستان‌آباد", "چاراویماق", "عجب‌شیر", "ملکان",
        "خداآفرین", "اسکو", "آذرشهر", "ترکمانچای",
    ],
    "آذربایجان غربی": [
        "ارومیه", "خوی", "بوکان", "مهاباد", "میاندوآب", "سلماس", "نقده", "پیرانشهر",
        "سردشت", "تکاب", "شاهین‌دژ", "چالدران", "ماکو", "پلدشت", "شوط", "چایپاره", "اشنویه",
    ],
    "بوشهر": [
        "بوشهر", "دشتستان", "تنگستان", "دشتی", "دیر", "کنگان", "عسلویه", "دیلم", "جم", "گناوه",
    ],
    "خوزستان": [
    ...
    "کرمان": [
        "کرمان", "رفسنجان", "سیرجان", "بم", "جیرفت", "زرند", "بردسیر", "بافت", "کهنوج",
        "شهربابک", "راور", "انار", "عنبرآباد", "رودبار جنوب", "منوجان", "قلعه‌گنج", "فهرج",
        "ریگان", "ارزوئیه", "نرماشیر", "رابر",
    ],
    "گیلان": [
        "رشت", "بندرانزلی", "لاهیجان", "لنگرود", "رودسر", "آستارا", "تالش", "صومعه‌سرا",
        "فومن", "شفت", "رودبار", "آستانه اشرفیه", "رضوانشهر", "ماسال", "املش", "سیاهکل",
    ],
    "مازندران": [
        "ساری", "بابل", "آمل", "قائم‌شهر", "بهشهر", "نور", "نوشهر", "چالوس", "تنکابن",
    ...
    "کرمانشاه": [
        "کرمانشاه", "اسلام‌آباد غرب", "پاوه", "سنقر", "کنگاور", "هرسین", "صحنه",
        "سرپل ذهاب", "قصرشیرین", "جوانرود", "گیلانغرب", "ثلاث باباجانی", "دالاهو",
    ],
    "هرمزگان": [
        "بندرعباس", "میناب", "بندرلنگه", "قشم", "رودان", "حاجی‌آباد", "بستک", "پارسیان",
        "بشاگرد", "ابوموسی", "خمیر", "جاسک", "سیریک",
    ],
    "سیستان و بلوچستان": [
        "زاهدان", "زابل", "ایرانشهر", "چابهار", "سراوان", "خاش", "نیک‌شهر", "کنارک",
    ...
    "لرستان": [
        "خرم‌آباد", "بروجرد", "دورود", "الیگودرز", "کوهدشت", "ازنا", "پل‌دختر",
    ...
    "گلستان": [
        "گرگان", "گنبدکاووس", "علی‌آباد کتول", "آق‌قلا", "کردکوی", "بندر ترکمن",
...
def resolve_target_user(text: str):
    """قبول می‌کنه: @username تلگرام، /user_CODE، یا آیدی عددی"""
    text = to_en_digits(text.strip())
    if text.startswith("@"):
        return None  # جستجوی یوزرنیم تلگرام نیازمند API جداست؛ در ادمین پنل با آیدی عددی یا کد ربات کار می‌کنیم
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if is_new and invited_by:
        add_coins(invited_by, REFERRAL_BONUS)
...
async def send_block_message(update: Update, context: ContextTypes.DEFAULT_TYPE, row):
    text = (
        "‼️ متاسفانه اکانت شما به دلیل گزارش کاربران\n"
    ...
    kb = InlineKeyboardMarkup([[InlineKeyboardButton("📝 ارسال تیکت اعتراض", callback_data="ticket:new")]])
    if update.message:
...
async def ticket_new_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    open_ticket = conn.execute(
        "SELECT 1 FROM tickets WHERE user_id=? AND status='open'", (user_id,)
    ).fetchone()
    conn.close()
    if open_ticket:
...
async def handle_ticket_text(update: Update, context: ContextTypes.DEFAULT_TYPE, text):
    ...
    kb = InlineKeyboardMarkup(
        [
            [
                InlineKeyboardButton("✅ پاسخ و رفع مسدودی", callback_data=f"tkok:{ticket_id}"),
                InlineKeyboardButton("❌ رد تیکت", callback_data=f"tkno:{ticket_id}"),
            ]
        ]
    )
...
async def ticket_decision_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if action == "tkok":
        ...
        conn.commit()
        await context.bot.send_message(
            ticket["user_id"], "❌ درخواست تجدیدنظر شما بررسی شد و رد شد. مسدودی همچنان فعال است."
        )
        await query.message.edit_text(query.message.text + "\n\n❌ تیکت رد شد.")
    conn.close()
    await query.answer()
...
def full_profile_keyboard(target_id):
    return InlineKeyboardMarkup(
        [
            ...
            [
                InlineKeyboardButton("🔒 بلاک کردن کاربر", callback_data=f"pblock:{target_id}"),
                InlineKeyboardButton("🚫 گزارش کاربر", callback_data=f"report:{target_id}"),
            ],
            [InlineKeyboardButton("➕ افزودن به مخاطبین", callback_data=f"addcontact:{target_id}")],
        ]
...
async def show_full_profile(update, context, viewer_id, target):
    ...
    online = "هم اکنون 👀 آنلایـــن" if target["in_chat_with"] or True else ""
    text_lines = [
    ...
    if target["photo_file_id"]:
        await context.bot.send_photo(viewer_id, target["photo_file_id"], caption=text, reply_markup=kb)
    else:
        await context.bot.send_message(viewer_id, text, reply_markup=kb)
...
async def show_coins(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    row = get_user(update.effective_user.id)
    text = (
        f"💰 سکه فعلی شما: {row['coins']}{vip_badge(row)}\n"
        "━━━━━━━━━━━━━━━━━\n\n"
...
async def show_vip(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if is_vip(row):
        ...
        kb = InlineKeyboardMarkup([[InlineKeyboardButton("🔄 تمدید اشتراک VIP", callback_data="vip:buy")]])
    else:
        kb = InlineKeyboardMarkup([[InlineKeyboardButton("💳 خرید اشتراک VIP", callback_data="vip:buy")]])
...
async def vip_callbacks(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == "vip:buy":
        ...
        rial = final_price * 10
        text = (
            "👑 لطفاً دقیقاً مبلغ زیر را برای فعال‌سازی اشتراک VIP به شماره کارت واریز کنید:\n\n"
            f"💰 مبلغ قابل پرداخت:\n`{final_price:,} تومان`\n`{rial:,} ریال`\n\n"
            f"💳 شماره کارت:\n`{CARD_NUMBER}`\n👤 {CARD_OWNER}\n\n"
...
async def free_text_router(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if state == "await_dm_text" and row["pending_search"] and row["pending_search"].startswith("dmtarget:"):
        ...
        if row["coins"] < cost:
            await update.message.reply_text(
                f"⚠️ سکه کافی نداری! ({cost} سکه مورد نیاز)", reply_markup=MAIN_MENU
            )
            return
        if cost:
...
def chat_menu_keyboard():
    return InlineKeyboardMarkup(
        [
            ...
            [InlineKeyboardButton("⛔️ پایان چت", callback_data="chat:end")],
        ]
    )
...
def searching_text(label: str, same_age_on: bool) -> str:
    ...
    return (
        f"🔎 در حال جستجوی مخاطب ناشناس شما...\n{label}\n\n"
...
def searching_keyboard(gender_filter: str, same_age_on: bool) -> InlineKeyboardMarkup:
    ...
    return InlineKeyboardMarkup(
        [
            ...
            [InlineKeyboardButton("❌ لغو جستجو", callback_data="cancelsearch")],
        ]
    )
    ...
    for cand in candidates:
        ...
        if want in ("any",) or want == me["gender"]:
            if gender_filter in ("any",) or gender_filter == cand_row["gender"]:
                # respect same-age preference on either side
...
async def search_callbacks(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("search:"):
        ...
        if mode == "nearby":
            if row["lat"] is None:
                await query.message.edit_text(
                    "⚠️ خطا: برای استفاده از این قسمت ابتدا باید موقعیت مکانی(GPS) خود را ثبت کنید!"
                )
        ...
        if mode == "province":
            await query.message.edit_text(
                "🗺 به کدوم استان وصلت کنم؟ انتخاب کن 👇", reply_markup=province_keyboard(prefix="matchprov")
            )
            await query.answer()
            return
    ...
    if data.startswith("matchprov:"):
        ...
        kb = InlineKeyboardMarkup(
            [
                [InlineKeyboardButton("👧 دختر باشه (۳ سکه)", callback_data=f"provgender:{province}:دختر")],
                [InlineKeyboardButton("🧑 پسر باشه (۳ سکه)", callback_data=f"provgender:{province}:پسر")],
                [InlineKeyboardButton("🎲 فرقی نمی‌کنه (رایگان)", callback_data=f"provgender:{province}:any")],
            ]
        )
    ...
    if data.startswith("provgender:"):
        ...
        kb = InlineKeyboardMarkup(
            [
                ...
                [InlineKeyboardButton("📜 لیست هم‌استانیای من (۱۰ سکه)", callback_data=f"provlist:{province}:{gender}")],
            ]
        )
        await query.message.edit_text(
    ...
    if data.startswith("provconn:"):
        _, province, gender = data.split(":")
        cost = 0 if (gender == "any" or is_vip(row)) else 3
        if row["coins"] < cost:
            await show_insufficient_coins(query, cost)
            await query.answer()
            return
    ...
    if data == "chat:end":
        await query.message.edit_text(
            ...
            reply_markup=InlineKeyboardMarkup(
                [
                    [
                        InlineKeyboardButton("▶️ ادامه‌ی چت", callback_data="chat:continue"),
                        InlineKeyboardButton("⛔️ اتمام چت", callback_data="chat:finish"),
                    ]
                ]
            ),
        )
    ...
    if action == "report":
        kb = InlineKeyboardMarkup(
            [
                ...
                [InlineKeyboardButton("🕵️ انتشار اطلاعات شخصی", callback_data=f"reportreason:{target_id}:حریم‌خصوصی")],
...
async def nearby_callbacks(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("nearbyf:") or data.startswith("nearbymore:"):
        ...
        if next_offset < len(results):
            buttons.append(
                [InlineKeyboardButton("➡️ مشاهده‌ی ادامه‌ی لیست", callback_data=f"nearbymore:{radius}:{gender}:{next_offset}")]
            )
...
async def usearch_callbacks(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    lines = [
        f"{r['age']} 😐 {r['name'] or '—'}{vip_badge(r)} /user_{r['username_code']} {r['province']}({r['city']})"
        for r in rows[:15]
    ]
...
async def user_code_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    blocked_me = conn.execute(
        "SELECT 1 FROM blocklist WHERE owner_id=? AND blocked_id=?",
        (target["user_id"], update.effective_user.id),
    ).fetchone()
    conn.close()
    if blocked_me:
...
async def full_profile_callbacks(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if action == "reqchat":
        ...
        await context.bot.send_message(
            ...
            reply_markup=InlineKeyboardMarkup(
                [
                    [
                        InlineKeyboardButton("✅ پذیرفتن", callback_data=f"pchat_ok:{user_id}"),
                        InlineKeyboardButton("❌ رد کردن", callback_data=f"pchat_no:{user_id}"),
                    ]
                ]
            ),
        )
    ...
    if action == "dm":
        ...
        await context.bot.send_message(
            user_id, f"📩 پیام دایرکتت رو برای /user_{target['username_code']} بنویس و بفرست:"
        )
        await query.answer()
        return
...
HELP_TOPICS = {
    ...
    "help_credit": (
        ...
        "- از جستجوی پسر یا جستجوی دختر استفاده کنید (2 سکه)\n"
        ...
        f"2️⃣ تکمیل پروفایل: با تکمیل ثبت‌نامت {PROFILE_COMPLETE_BONUS} سکه رایگان می‌گیری 👤\n\n"
        f"3️⃣ معرفی دوستان: بنر لینک⚡️ مخصوص خودت (/link) رو برای دوستات بفرست و به ازای هرنفر که "
    ...
    kb = InlineKeyboardMarkup(
        [
            [
                ...
                InlineKeyboardButton("↩️ انصراف", callback_data="delacc:no"),
            ]
        ]
    )
...
def admin_panel_keyboard(owner):
    rows = [
        [InlineKeyboardButton("🪙 مدیریت سکه کاربر", callback_data="admin:coin")],
        [InlineKeyboardButton("🔒 مسدود / رفع مسدود کاربر", callback_data="admin:block")],
        [InlineKeyboardButton("📋 تیکت‌های باز", callback_data="admin:tickets")],
    ]
    if owner:
        rows.append([InlineKeyboardButton("➕ افزودن ادمین جدید", callback_data="admin:addadmin")])
    return InlineKeyboardMarkup(rows)
...
async def admin_panel_callbacks(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "admin:coin":
        ...
        await query.message.edit_text(
            "🪙 آیدی عددی یا کد کاربر (/user_XXXX) شخص مورد نظر رو ارسال کن:"
        )
        await query.answer()
        return
    ...
    if data == "admin:addadmin":
        ...
        await query.message.edit_text(
            "➕ آیدی عددی یا کد کاربر (/user_XXXX) شخصی که می‌خوای ادمین کنی رو ارسال کن:"
        )
        await query.answer()
        return
    ...
    if data == "admin:tickets":
        ...
        for t in rows:
            ...
            kb = InlineKeyboardMarkup(
                [
                    [
                        InlineKeyboardButton("✅ پاسخ و رفع مسدودی", callback_data=f"tkok:{t['id']}"),
                        InlineKeyboardButton("❌ رد تیکت", callback_data=f"tkno:{t['id']}"),
                    ]
                ]
            )
            await context.bot.send_message(
    ...
    if data.startswith("admin:coinamt:"):
        ...
        add_coins(target_id, delta)
        newrow = get_user(target_id)
        await query.message.edit_text(
            f"✅ انجام شد. سکه کاربر /user_{newrow['username_code']} اکنون: {newrow['coins']}"
        )
        try:
            await context.bot.send_message(
                target_id,
    ...
    if data.startswith("admin:coincustom:"):
        ...
        await query.message.edit_text(
            "🔢 مقدار سکه رو وارد کن (برای کم کردن، عدد منفی بفرست، مثلاً -20):"
        )
        await query.answer()
        return
...
async def handle_admin_text(update: Update, context: ContextTypes.DEFAULT_TYPE, row, text):
    ...
    if state == "admin_wait_target":
        ...
        if not target:
            await update.message.reply_text(
                "⚠️ کاربری پیدا نشد. دوباره آیدی عددی یا کد /user_XXXX رو بفرست (یا برای لغو بنویس «بازگشت»):"
            )
            return
        update_user(user_id, state=None, pending_search=None)
        ...
        if mode == "coin":
            info = (
                f"👤 کاربر: {target['name'] or '—'}{vip_badge(target)}\n"
            ...
            kb = InlineKeyboardMarkup(
                [
                    [
                        InlineKeyboardButton("➕10", callback_data=f"admin:coinamt:{target['user_id']}:10"),
                        InlineKeyboardButton("➕50", callback_data=f"admin:coinamt:{target['user_id']}:50"),
                        InlineKeyboardButton("➕100", callback_data=f"admin:coinamt:{target['user_id']}:100"),
                    ],
                    [
                        InlineKeyboardButton("➖10", callback_data=f"admin:coinamt:{target['user_id']}:-10"),
                        InlineKeyboardButton("➖50", callback_data=f"admin:coinamt:{target['user_id']}:-50"),
        ...
        if mode == "block":
            if target["is_blocked"]:
                ...
                kb = InlineKeyboardMarkup(
                    [[InlineKeyboardButton("✅ رفع مسدودی", callback_data=f"admin:unblockdo:{target['user_id']}")]]
                )
            else:
                info = f"👤 کاربر /user_{target['username_code']} در حال حاضر آزاد است."
                kb = InlineKeyboardMarkup(
                    [[InlineKeyboardButton("🔒 مسدود کردن", callback_data=f"admin:blockdo:{target['user_id']}")]]
                )
            await update.message.reply_text(info, reply_markup=kb)
            return
        ...
        if mode == "addadmin":
            ...
            try:
                await context.bot.send_message(
                    target["user_id"], "🎉 شما توسط مالک ربات به عنوان ادمین ایزی گپ منصوب شدید!"
                )
            except Exception:
                pass
            return
    ...
    if state == "admin_wait_block_reason":
        ...
        try:
            await context.bot.send_message(
                ...
                reply_markup=InlineKeyboardMarkup(
                    [[InlineKeyboardButton("📝 ارسال تیکت اعتراض", callback_data="ticket:new")]]
                ),
            )
        except Exception:
            pass
...
async def callback_router(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # کاربران مسدود شده فقط اجازه‌ی ارسال تیکت دارن
    row = get_user(update.effective_user.id)
    ...
    if data == "back:main":
        await profile_callbacks(update, context)
        return
    if data.startswith(("search:", "matchprov:", "provgender:", "provconn:", "provlist:", "sameage:toggle:")) or data == "cancelsearch":
...
async def generic_command_catch(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # اگر ربات منتظر پاسخ کاربر (آیدی/کد) بود، اول اون رو پردازش کن؛
    # وگرنه پیام‌هایی مثل /user_XXXX توسط این هندلر قاپیده می‌شن و state هیچوقت پاک نمی‌شه (باگ لوپ بی‌نهایت)
    if row:
...
def main():
    ...
    app.add_handler(CommandHandler("of", off_command))
