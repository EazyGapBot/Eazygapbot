...
def profile_completion_keyboard():
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("✨📝 تکمیل پروفایل 📝✨", callback_data="profile_complete_start")]
    ])
...
def coin_shop_keyboard():
    ...
    for coins, price in COIN_PACKAGES:
        rows.append([InlineKeyboardButton(f"خرید {coins} سکه: {price:,} تومان", callback_data=f"buy:{coins}:{price}")])
    return InlineKeyboardMarkup(rows)
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if step == "name":
        ...
        await ask_phone(update.message, context)
...
async def registration_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("city:") and row["reg_step"] == "city":
        city = data.split(":", 1)[1]
        update_user(user_id, city=city, reg_step="phone")
        await query.message.edit_text("✅ شهر شما ثبت شد.")
        await ask_phone(query.message, context)
        await query.answer()
        return
    await query.answer()
...
async def show_profile(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await send_profile_photo(context.bot, update.effective_user.id, row, text, profile_keyboard())
...
async def chat_menu_callbacks(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "chat:finish":
        ...
        if partner_id:
            ...
            end_text = (f"چت شما با «آیدی کاربر(/user_{row['username_code']})» توسط کاربر مقابل قطع شد\n\n"
    ...
    if state == "admin_wait_target":
        ...
        if mode == "block":
            if target["is_blocked"]:
                ...
                kb = InlineKeyboardMarkup([[InlineKeyboardButton("✅ رفع مسدودی", callback_data=f"admin:unblockdo:{target['user_id']}")]])
            else:
                info = f"👤 کاربر /user_{target['username_code']} در حال حاضر آزاد است."
                kb = InlineKeyboardMarkup([[InlineKeyboardButton("🔒 مسدود کردن", callback_data=f"admin:blockdo:{target['user_id']}")]])
            await update.message.reply_text(info, reply_markup=kb)
            return
    ...
    if state == "admin_wait_block_reason":
        ...
        try:
            await context.bot.send_message(target_id,
                ...
                reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("📝 ارسال تیکت اعتراض", callback_data="ticket:new")]]),
            )
        except Exception:
            pass
        return
...
async def callback_router(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith(("age:", "province:", "city:")):
        await registration_callback(update, context)
        return
    if data.startswith(("editf:", "cityedit:", "edit_profile", "back:profile", "my_gps", "my_likers", "my_contacts", "profile_complete_start")):
